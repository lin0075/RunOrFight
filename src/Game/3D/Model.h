    ...
    void updateBoundingBox()
    {
        ...
        for (const auto &mesh : meshes)
        {
            for (const auto &vertex : mesh.vertices)
            {
                ...
                min = glm::min(min, scaledPosition);
                max = glm::max(max, scaledPosition);
            }
        }

        boundingBox.min = min + position;
        boundingBox.max = max + position;
    }
    void startCharge(float duration)
    {
        isCharging = true;
        chargeEndTime = (float)glfwGetTime() + duration;
    }

    void updateCharging()
    {
        if (isCharging && glfwGetTime() > chargeEndTime)
        {
            isCharging = false;
        }
    }
